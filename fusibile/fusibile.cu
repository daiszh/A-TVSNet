...
__global__ void fusibile (GlobalState &gs, int ref_camera)
{
    ...
    for ( int i = 0; i < camParams.viewSelectionSubsetNumber; i++ ) {
        ...
        if (tmp_pt.x >=0 &&
            ...
            // tmp_normal_and_depth   = tex2D<float4> (gs.normals_depths[idxCurr], tmp_pt.x, tmp_pt.y);
            ...
            if ((fabsf(depth_disp - tmp_normal_and_depth_disp) / depth_disp) < gs.params->depthThresh) {
...
void copy_point_cloud_to_host(GlobalState &gs, int cam, PointCloudList &pc_list, int num_views=0)
{
    bool print_log = true;
    if (num_views-1 > cam)
        print_log = false;
 ...
 */    printf("Grid size initrand is grid: %d-%d block: %d-%d\n", grid_size_initrand.x, grid_size_initrand.y, block_size_initrand.x, block_size_initrand.y);
    ...
    for (int cam=0; cam<num_views; cam++) {
        ...
        copy_point_cloud_to_host(gs, cam, pc_list, num_views); // slower but saves memory
        cudaDeviceSynchronize();
    }
